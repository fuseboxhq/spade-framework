...
Use the flag (`--deliver`, `--plan`, `--scope`, `--stub`), else `autonomy.default`, else ask once through `request_user_input when available, otherwise a concise direct question` with the options Deliver, Plan, Scope, Stub.
...
For broad mechanical work, hand slices to isolated agents through `spawn_agent with a self-contained prompt and no inherited conversation`, each in its own worktree, and check each one's evidence before accepting it.
...
On a tripwire, record it in the Plan's Halts section (and as a comment on the Scope issue in `linear` mode), ask the human through `request_user_input when available, otherwise a concise direct question`, and wait.
