...
Dispatch the work through `codex exec --sandbox read-only --ignore-user-config --ephemeral with the canonical researcher prompt` to the `spade-researcher` agent.
...
Before any post, ask for that approval through `request_user_input when available, otherwise a concise direct question` and name the target issue.
