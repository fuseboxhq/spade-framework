...
When public safety is unclear, ask through `request_user_input when available, otherwise a concise direct question` with exactly these choices:
...
Use `request_user_input when available, otherwise a concise direct question` to confirm before setting `status: archived` or revising an active entry.
