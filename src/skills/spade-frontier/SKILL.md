...
Ask through {{SPADE_ASK_USER}} with exactly:
...
If more than one question is visible, ask through {{SPADE_ASK_USER}} with the visible question ids and concise titles plus `Cancel`.
...
Use {{SPADE_ASK_USER}} with the viable options plus `Defer` and `Cancel`.
