...
prompt the human via **`{{SPADE_ASK_USER}}`** (per `docs/FRAMEWORK.md`
...
Ask the human via **`{{SPADE_ASK_USER}}`** whether to set up handoff now:
...
If they opt in, ask two structured questions (`{{SPADE_ASK_USER}}`):
...
the human (`{{SPADE_ASK_USER}}`: *iTerm2* / *Terminal.app*) and write it to
`.spade/handoff.local`:
...
Ask the human via **`{{SPADE_ASK_USER}}`** whether this project is tracked on
Horizon:
...
2. Ask the human to confirm the mode via **`{{SPADE_ASK_USER}}`** (per
