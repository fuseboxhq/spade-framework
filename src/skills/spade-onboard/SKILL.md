...
For a new `.spade/config`, use `{{SPADE_ASK_USER}}` to confirm `linear` when Linear is available and the human selects the team and project.
...
Show the proposed verification procedure and use `{{SPADE_ASK_USER}}` to confirm every command before writing it.
