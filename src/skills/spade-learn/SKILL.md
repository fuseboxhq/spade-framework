...
When public safety is unclear, ask through `{{SPADE_ASK_USER}}` with exactly these choices:
...
Use `{{SPADE_ASK_USER}}` to confirm before setting `status: archived` or revising an active entry.
