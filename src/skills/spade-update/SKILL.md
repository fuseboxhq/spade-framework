...
Ask through `{{SPADE_ASK_USER}}` whether to approve that exact SHA or skip.
...
Show the exact SHA and summary, then ask through `{{SPADE_ASK_USER}}` to approve it before running setup with `SPADE_APPROVED_SHA`.

If an installation is damaged, show the failed diagnostics first.
Never remove `~/.spade` without explicit structured confirmation through `{{SPADE_ASK_USER}}` that names that exact path and says the removal is destructive.
