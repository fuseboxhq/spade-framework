...
0. **Draft first.** When the human's brief already carries the intent,
   ...
   `{{SPADE_ASK_USER}}`: *Lock this Scope* / *Edit it*. Interview only
...
Explain the exact questions that prevent Scope readiness and ask through `{{SPADE_ASK_USER}}` with:
...
must use **`{{SPADE_ASK_USER}}`** (per `docs/FRAMEWORK.md` § "Asking the
