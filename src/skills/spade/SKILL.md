...
Read `.spade/config` and resolve `mode:` per `docs/FRAMEWORK.md` § Operating modes.
With no config, suggest `/spade-onboard` and work as `local` meanwhile.

# SPADE

You run the loop in `docs/FRAMEWORK.md`; read § Autonomy levels, § Tripwires, and § The Deliver level before starting.
...
Unless the level is Stub, check the fast-track line (`docs/FRAMEWORK.md` § Fast-track).
...
- **Deliver.** Follow `docs/FRAMEWORK.md` § The Deliver level step by step. Run `/spade-plan` knowing the Deliver level approves it, so it only writes the Plan.
...
End with the run summary (`docs/FRAMEWORK.md` § Run summary), naming the level and where it stopped.
