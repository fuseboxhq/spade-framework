...
Show a concise validation matrix and ask through `{{SPADE_ASK_USER}}` with exactly two choices:
...
After classification, offer only the applicable choices through `{{SPADE_ASK_USER}}`:
...
When every change is consistent with the recorded current work, show the evidence and ask through `{{SPADE_ASK_USER}}` with exactly:
