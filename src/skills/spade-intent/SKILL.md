...
said "set up our intent doc"), confirm the mode with `{{SPADE_ASK_USER}}` rather
than guessing.

### Create Mode

Walk the human through all six sections from scratch (or fill the scaffolded
template). Before starting, offer — via `{{SPADE_ASK_USER}}` — to draft a starting
...
`/spade-evaluate`). Use `{{SPADE_ASK_USER}}` to scope the edit:
...
## Decision Prompts ({{SPADE_ASK_USER}})
...
decisions** this skill makes along the way use `{{SPADE_ASK_USER}}` (per
