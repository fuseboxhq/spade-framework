---
name: spade-research
description: Landscape research on a topic via an isolated {{SPADE_AGENT_MODEL}} subagent with read-only research tools. Use when the human says "properly research this", "look into X", "check the prior art", "second opinion on the landscape", "what does the SOTA look like for X", or asks an open question that needs external fact-finding such as libraries, frameworks, benchmarks, postmortems, or comparisons. Returns a structured findings report and optionally posts to a Linear parent issue with explicit human consent. Callable at any time and not tied to a SPADE phase. Also matches the explicit slash-command form `/spade-research`.
---
...
`spade-researcher` subagent context ({{SPADE_AGENT_MODEL}}, read-only tools,
...
asks the human via `{{SPADE_ASK_USER}}` (per the framework's "Asking
...
3. **Spawn the researcher through {{SPADE_READ_ONLY_RESEARCH}}.** Pass:
...
6. **If scoped, ask the consent question via `{{SPADE_ASK_USER}}`.**
