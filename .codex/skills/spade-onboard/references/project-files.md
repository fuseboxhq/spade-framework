...
prompt the human via **`request_user_input when available, otherwise a concise direct question`** (per `docs/FRAMEWORK.md`
...
Ask the human via **`request_user_input when available, otherwise a concise direct question`** whether to set up handoff now:
...
If they opt in, ask two structured questions (`request_user_input when available, otherwise a concise direct question`):
...
the human (`request_user_input when available, otherwise a concise direct question`: *iTerm2* / *Terminal.app*) and write it to
`.spade/handoff.local`:
...
Ask the human via **`request_user_input when available, otherwise a concise direct question`** whether this project is tracked on
Horizon:
...
2. Ask the human to confirm the mode via **`request_user_input when available, otherwise a concise direct question`** (per
