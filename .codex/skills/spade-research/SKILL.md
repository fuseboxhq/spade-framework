---
name: spade-research
description: Landscape research on a topic via an isolated host-default subagent with read-only research tools. Use when the human says "properly research this", "look into X", "check the prior art", "second opinion on the landscape", "what does the SOTA look like for X", or asks an open question that needs external fact-finding such as libraries, frameworks, benchmarks, postmortems, or comparisons. Returns a structured findings report and optionally posts to a Linear parent issue with explicit human consent. Callable at any time and not tied to a SPADE phase. Also matches the explicit slash-command form `/spade-research`.
---
...
`spade-researcher` subagent context (host-default, read-only tools,
...
asks the human via `request_user_input when available, otherwise a concise direct question` (per the framework's "Asking
...
3. **Spawn the researcher through codex exec --sandbox read-only --ignore-user-config --ephemeral with the canonical researcher prompt.** Pass:
...
6. **If scoped, ask the consent question via `request_user_input when available, otherwise a concise direct question`.**
