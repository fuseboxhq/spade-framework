...
If every prerequisite agrees, show the validation matrix and offer `Continue from <boundary>` or `Cancel` through `request_user_input when available, otherwise a concise direct question`.
...
3. **The picker.** Else, present the single-select picker via
   `request_user_input when available, otherwise a concise direct question` — exactly four options, in this fixed order, one
   selection:
...
- **Deliver.** Run Plan's chain first — `/spade-scope` (record `scope:*`),
  ...
  **auto-approve and continue** per FRAMEWORK.md § "The Deliver level":
  0. **Arm the guard.** Through `exec_command`, check whether
...
§ Surfacing channel) — the in-session `request_user_input when available, otherwise a concise direct question` prompt AND a durable
