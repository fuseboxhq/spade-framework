...
Show a concise validation matrix and ask through `request_user_input when available, otherwise a concise direct question` with exactly two choices:
...
After classification, offer only the applicable choices through `request_user_input when available, otherwise a concise direct question`:
...
When every change is consistent with the recorded current work, show the evidence and ask through `request_user_input when available, otherwise a concise direct question` with exactly:
