...
human via `request_user_input when available, otherwise a concise direct question` (per `docs/FRAMEWORK.md` § "Asking the
...
`git show HEAD:.spade/config` through `exec_command`, never from the
...
- **4** — worktree collision: `--cwd` is the same git worktree as this
  ...
  branch. Confirm via `request_user_input when available, otherwise a concise direct question`:
