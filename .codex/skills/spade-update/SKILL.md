...
Ask through `request_user_input when available, otherwise a concise direct question` whether to approve that exact SHA or skip.
...
Show the exact SHA and summary, then ask through `request_user_input when available, otherwise a concise direct question` to approve it before running setup with `SPADE_APPROVED_SHA`.

If an installation is damaged, show the failed diagnostics first.
Never remove `~/.spade` without explicit structured confirmation through `request_user_input when available, otherwise a concise direct question` that names that exact path and says the removal is destructive.
