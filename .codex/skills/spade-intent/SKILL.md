...
said "set up our intent doc"), confirm the mode with `request_user_input when available, otherwise a concise direct question` rather
than guessing.

### Create Mode

Walk the human through all six sections from scratch (or fill the scaffolded
template). Before starting, offer — via `request_user_input when available, otherwise a concise direct question` — to draft a starting
...
`/spade-evaluate`). Use `request_user_input when available, otherwise a concise direct question` to scope the edit:
...
decisions** this skill makes along the way use `request_user_input when available, otherwise a concise direct question` (per
