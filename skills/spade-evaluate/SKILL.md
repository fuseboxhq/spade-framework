...
When the work has a PR, write `.spade/guard/reviewed-head-<pr>` through `Bash` as one line, `<reviewed head sha> <VERDICT>`, so the merge guard can check it (`references/FRAMEWORK.md` § Mechanical guards).
