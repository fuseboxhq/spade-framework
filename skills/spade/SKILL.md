...
Use the flag (`--deliver`, `--plan`, `--scope`, `--stub`), else `autonomy.default`, else ask once through `AskUserQuestion` with the options Deliver, Plan, Scope, Stub.
...
For broad mechanical work, hand slices to isolated agents through `Task`, each in its own worktree, and check each one's evidence before accepting it.
...
On a tripwire, record it in the Plan's Halts section (and as a comment on the Scope issue in `linear` mode), ask the human through `AskUserQuestion`, and wait.
