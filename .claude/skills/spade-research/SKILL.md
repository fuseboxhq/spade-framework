...
Dispatch the work through `registered spade-researcher agent with the declared read-only tool allowlist` to the `spade-researcher` agent.
...
Before any post, ask for that approval through `AskUserQuestion` and name the target issue.
