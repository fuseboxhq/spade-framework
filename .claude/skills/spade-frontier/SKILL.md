...
Ask through AskUserQuestion with exactly:
...
If more than one question is visible, ask through AskUserQuestion with the visible question ids and concise titles plus `Cancel`.
...
Use AskUserQuestion with the viable options plus `Defer` and `Cancel`.
