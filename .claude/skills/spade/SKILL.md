...
If every prerequisite agrees, show the validation matrix and offer `Continue from <boundary>` or `Cancel` through `AskUserQuestion`.
...
3. **The picker.** Else, present the single-select picker via
   `AskUserQuestion` — exactly four options, in this fixed order, one
   selection:
...
- **Deliver.** Run Plan's chain first — `/spade-scope` (record `scope:*`),
  ...
  **auto-approve and continue** per FRAMEWORK.md § "The Deliver level":
  0. **Arm the guard.** Through `Bash`, check whether
...
§ Surfacing channel) — the in-session `AskUserQuestion` prompt AND a durable
