...
When you record it and the work has a PR, write `.spade/guard/reviewed-head-<pr>` through `Bash` as one line, `<reviewed head sha> <VERDICT>`, so the merge guard can check it (`references/FRAMEWORK.md` § Mechanical guards).

Otherwise ask the human through `AskUserQuestion`: *PASS*, *PARTIAL*, or *FAIL*, with your recommendation first and a plain statement when the evidence does not support PASS.
