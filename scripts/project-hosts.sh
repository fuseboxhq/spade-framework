...
validate_sources() {
    ...
    actual_personas=$(find "$SOURCE_ROOT/agents" -maxdepth 1 -type f -name 'spade-reviewer.md' -exec basename {} .md \; | LC_ALL=C sort)
...
render_skills() {
    ...
    # Consumer repositories have no copy of the framework reference, so every
    # skill ships one next to itself as references/FRAMEWORK.md.
    find "$SOURCE_ROOT/skills" -mindepth 1 -maxdepth 1 -type d | LC_ALL=C sort | while IFS= read -r skill_dir; do
        mkdir -p "$destination_root/$(basename "$skill_dir")/references"
        cp "$REPO_ROOT/docs/FRAMEWORK.md" "$destination_root/$(basename "$skill_dir")/references/FRAMEWORK.md"
    done
...
generate_tree() {
    ...
    for source_file in "$SOURCE_ROOT"/agents/spade-reviewer.md; do
