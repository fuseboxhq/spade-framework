...
- **Deliver-level exception.** The Deliver autonomy level (`/spade`) records a
  machine-attributed auto-approval and continues to code and an open PR, halting
  before merge. This does not remove the gate — it **relocates it to the PR**,
  which a human reviews (the approach summary and the diff) and merges. Deliver
  never merges. Every other path keeps the pre-code STOP gate above.
- **Independent second opinion (optional).** The human may request
  `/spade-review` for an independent perspective before deciding. A triage
  step casts persona subagents for the change — five canonical personas
  (scope-guardian, architecture-strategist, security-lens,
  adversarial-reviewer, alternatives-analyst) plus ad-hoc personas for
  dimensions no canonical lane covers, under drop-with-cause — merges their
  structured findings by convergence into a single report, and presents it.
