...
| Skill | What it does |
|-------|-------------|
| `/spade` | Orchestrator entry point: Deliver, Plan, Scope, or Stub; Deliver includes project-native checks, a fixed-range two-axis review, and merge by policy when guards are live |
...
| `/spade-plan` | Generate a structured Plan from a Scope; each task is one strict card (What / Done when / How / Verify / Needs / Blocks / Who); surfaces prior learnings |
...
- Never mark a parent issue Done.
  The human owns Done and Ship; the agent records the Evaluate verdict when
  every criterion is machine-verifiable
